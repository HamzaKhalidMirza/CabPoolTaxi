...
export class PhoneVerificationPage implements OnInit, AfterViewInit {
  ...
  getVerificationCode(sendStatus?: string) {
    ...
    this.clientAuthService.getVerificationCode(this.clientObj)
      .subscribe(
        response => {
          this.loading = false;
  ...
  verifyCode() {
    ...
    this.clientAuthService.verifyCode(this.clientObj)
      .subscribe(
        response => {
          this.loading = false;
          this.router.navigateByUrl('/app-starter-auth/signup/email');
        },
        (error: AppError) => {
          this.loading = false;
